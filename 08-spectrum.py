...
objs = [container]
objs.append(box(pos=vector(4*d+L,hslit/2+dy/2,0), size=vector(0.01*d,dy,2*zblue+2*(N-1)*wslit+wslit+2*dy), color=gray))
objs.append(box(pos=vector(4*d+L,-hslit/2-dy/2,0), size=vector(0.01*d,dy,2*zblue+2*(N-1)*wslit+wslit+2*dy), color=gray))
objs.append(box(pos=vector(4*d+L,0,zblue+(N-1)*wslit+wslit/2+dy/2), size=vector(0.01*d,hslit,dy), color=gray))
objs.append(box(pos=vector(4*d+L,0,-zblue-(N-1)*wslit-wslit/2-dy/2), size=vector(0.01*d,hslit,dy), color=gray))
objs.append(box(pos=vector(4*d+L,0,(zblue+wslit/2)/2), size=vector(0.01*d,hslit,(zblue-wslit/2)), color=gray))
objs.append(box(pos=vector(4*d+L,0,(-zblue-wslit/2)/2), size=vector(0.01*d,hslit,(zblue-wslit/2)), color=gray))
apparatus = compound(objs)

slit = box(pos=vector(4*d,0,0), size=vector(0.3*d,hslit,wslit), color=gray)
center = box(pos=vector(4*d+L,0,0), size=vector(0.015*d,hslit,wslit), color=gray) # center of spectrum screen
...
#gobjects.append(label(pos=vector(0,1.2*d,-1.8*d), text='Diffraction Grating', visible=False))
...
white = []
for nn in range(N):
    left.append(box(pos=center.pos+vector(0,0,zblue+nn*wslit), size=vector(0.01*d,hslit,wslitadjust), color=gray))
    leftray.append(box(pos=(grating.pos+left[-1].pos)/2, size=vector(mag(left[-1].pos-grating.pos),hslit,wslit), color=gray, visible=0))
    leftray[-1].axis = left[-1].pos-grating.pos
    right.append(box(pos=center.pos+vector(0,0,-zblue-nn*wslit), size=vector(0.01*d,hslit,wslitadjust), color=gray))
    rightray.append(box(pos=(grating.pos+right[-1].pos)/2, size=vector(mag(right[-1].pos-grating.pos),hslit,wslit), color=gray, visible=0))
