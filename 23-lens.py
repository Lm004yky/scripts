...
class lens:
    ...
    def setup(self, pos, R1, R2, radius, n):
        ...
        lensaxis = curve(pos=[vector(-3*endx,0,0), vector(3*endx,0,0)], radius=thick, color=color.blue)
        self.center1 = vector(self.pos.x+R1*cos(maxangle1)-self.spacer1,self.pos.y,0)
        self.leftsurface = curve(radius=thick, color=color.black)
        self.rightsurface = curve(radius=thick, color=color.black)
        ...
        if self.spacer1+self.spacer2 > 0:
            ...
            if self.rightsurface.point(Rlen-1).pos.y < self.rightsurface.point(0).pos.y:
                rightstart = Rlen-1
                rightend = 0
            self.top = curve(pos=[self.leftsurface.point(leftend).pos, self.rightsurface.point(rightend).pos], radius=thick)
            self.bottom = curve(pos=[self.leftsurface.point(leftstart).pos, self.rightsurface.point(rightstart).pos], radius=thick)
